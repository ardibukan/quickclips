        ...
        <div className="container mx-auto px-6">
          <div className="text-center mb-12">
            ...
            <p className="max-w-2xl mx-auto text-lg md:text-xl text-gray-600 dark:text-gray-400">
              Stay updated with the latest advancements, research, and developer news from the forefront of AI.
            </p>
          </div>
                ...
                <div key={index} className="w-full flex-shrink-0 px-2">
                  <div className="bg-light-card dark:bg-dark-card border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-lg dark:shadow-brand-blue/10">
                    <div className="relative mb-4">
                      ...
                      <button 
                        className="absolute inset-0 bg-black/20 rounded-2xl flex items-center justify-center cursor-pointer group w-full"
                        onClick={() => setActiveVideoUrl(item.video)}
                        aria-label={`Play video for ${item.title}`}
                      >
                          <div className="bg-black/50 p-3 rounded-full transition-transform duration-300 group-hover:scale-110">
            ...
            {carouselItems.map((_, index) => (
              <button
                ...
                className={`rounded-full transition-all duration-500 ease-in-out ${
                  activeIndex === index
                    ? 'w-9 h-3 bg-transparent relative overflow-hidden border border-gray-500 dark:border-gray-400'
